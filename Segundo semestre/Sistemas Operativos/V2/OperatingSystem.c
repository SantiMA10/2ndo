...
int OperatingSystem_LongTermScheduler() {
	...
	while (userProgramsList[i]!=NULL && i<USERPROGRAMSMAXNUMBER) {
		...
		if(PID == NOFREEENTRY){
			ComputerSystem_DebugMessage(ERROR,"sdsRsRsRs","[",Clock_GetTime(),"] ","ERROR: There are not free entries in the process table for the program [",userProgramsList[i]->executableName,"]\n");
		}
		else if (PID == PROGRAMDOESNOTEXIST){
			ComputerSystem_DebugMessage(ERROR,"sdsRsRsRs","[",Clock_GetTime(),"] ","ERROR: Program [",userProgramsList[i]->executableName,"] is not valid (it does not exist)\n");
		}
		else if (PID == PROGRAMNOTVALID){
			ComputerSystem_DebugMessage(ERROR,"sdsRsRsRs","[",Clock_GetTime(),"] ","ERROR: Program [",userProgramsList[i]->executableName,"] is not valid (invalid size or priority)\n");
		}
		else if (PID == TOOBIGPROCESS){
			ComputerSystem_DebugMessage(ERROR,"sdsRsRsRs","[",Clock_GetTime(),"] ","ERROR: Program [",userProgramsList[i]->executableName,"] is too big\n");
		}
		else{
		  numberOfSuccessfullyCreatedProcesses++;
		  ComputerSystem_DebugMessage(INIT,"sdsGsGdGsGsGs","[",Clock_GetTime(),"] ","Process [",PID,"] created from program [",userProgramsList[i]->executableName,"]\n");
		}
		i++;
	}
...
int OperatingSystem_ShortTermScheduler() {
	...
	if(selectedProcess == NOPROCESS && numberOfSleepingProcesses > 0){
...
void OperatingSystem_HandleException() {
  ...
  ComputerSystem_DebugMessage(SYSPROC,"sdsRsRdRs","[",Clock_GetTime(),"] ","Process [",executingProcessID,"] has generated an exception and is terminating\n");
...
void OperatingSystem_HandleClockInterrupt() {
	...
	int PIDSleeping, actualPID, i; //Recorrer cola de dormidos
	...
	for(i = 0; i < numberOfSleepingProcesses; i++){
		...
		if(processTable[PIDSleeping].whenToWakeUp == numberOfClockInterrupts){
			...
			if(processTable[actualPID].priority > processTable[PIDSleeping].priority){
				...
				OperatingSystem_ExtractFromReadyToRun(processTable[PIDSleeping].queueID);
				OperatingSystem_PreemptRunningProcess();
				OperatingSystem_Dispatch(PIDSleeping);
