...
void Processor_FetchInstruction() {
	...
	if (MMU_readMemory()==MMU_SUCCESS) {
	  ...
	  ComputerSystem_DebugMessage(HARDWARE,"sdsCcCsCdCsCd","[",Clock_GetTime(),"] ",registerIR_CPU.operationCode," ",registerIR_CPU.operand1," ",registerIR_CPU.operand2);
	}
	else 
...
void Processor_DecodeAndExecuteInstruction() {
	...
	ComputerSystem_DebugMessage(HARDWARE,"sGdsRdsRdsRds"," (PID: ",OperatingSystem_GetExecutingProcessID(),", PC: ",registerPC_CPU,", Accumulator: ",registerAccumulator_CPU,", PSW: ",registerPSW_CPU,")\n");
}
